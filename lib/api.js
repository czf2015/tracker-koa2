 ...
 * @param res 返回请求
 */
function getDataFromPath(path, method, params, res) {
    if (path) {
        fs.access(
            // 提取请求路径中的js文件
            path.substring(1) + '.js',
            ...
            function (err) {
                if (!err) {
                    ...
                    try {
                        addApiResult(res, require('..' + path).getData(method, params));
                    } catch (e) {
                        console.error(e.stack);
                        res.status(500).send(path + ' has an error,please check the code.');
                    }
                } else {
                    addApiResult(res);
                }
